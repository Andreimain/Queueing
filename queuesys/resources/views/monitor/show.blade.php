...
<body class="bg-gray-100 min-h-screen p-8">
    ...
    <!-- Title -->
    <h1 class="text-4xl font-bold mb-8 text-center">{{ $office }} Queue Display</h1>
            ...
            <div id="current-queue" class="text-center">
                @if($currentQueue)
                    <div class="text-6xl font-bold text-green-600">
                        {{ $currentQueue->queue_number }} - {{ $currentQueue->first_name }} {{ $currentQueue->last_name }}
                    </div>
                @else
        ...
        <div class="border p-10 rounded-2xl shadow bg-white h-[80vh] overflow-y-auto">
            ...
            <ul id="upcoming-queues" class="list-disc pl-8 space-y-4 text-3xl">
                @forelse($upcomingQueues as $queue)
                    <li>{{ $queue->queue_number }} - {{ $queue->first_name }} {{ $queue->last_name }}</li>
                @empty
    ...
    <script>
        ...
        function refreshQueue() {
            fetch("{{ route('monitor.data', $office) }}")
                ...
                .then(data => {
                    ...
                    if (data.currentQueue) {
                        currentQueueDiv.innerHTML =
                            `<div class="text-6xl font-bold text-green-600">
                                ${escapeHtml(data.currentQueue.queue_number)} -
                                ${escapeHtml(data.currentQueue.first_name)} ${escapeHtml(data.currentQueue.last_name)}
                            </div>`;
                    } else {
                        currentQueueDiv.innerHTML =
                            `<div class="text-3xl text-gray-500">
                    ...
                    if (data.upcomingQueues.length > 0) {
                        upcomingList.innerHTML = data.upcomingQueues.map(q =>
                            `<li class="text-3xl">${escapeHtml(q.queue_number)} - ${escapeHtml(q.first_name)} ${escapeHtml(q.last_name)}</li>`
                        ).join('');
                    } else {
