<x-app-layout>
    ...
    @php
        $transferStaff = $allOffices->mapWithKeys(function ($office) {
            return [
                    ...
                    <div class="flex justify-center flex-wrap gap-4 mb-6">
                        @forelse ($serving as $visitor)
                            ...
                            <div class="bg-emerald-50 rounded-lg shadow-md p-6 w-64 text-center border-4 {{ $borderColor }}"
                                ...
                                {{-- Transfer Form --}}
                                @if (auth()->user()->isStaff() && $visitor->cashier_id === auth()->id())
                        ...
                        @if ($waiting->count())
                            @if (auth()->user()->isStaff())
                                <form id="skipForm" method="POST"
                                    action="{{ route('office.queue.skip', $office->id) }}">
                                    @csrf
                                    <ul class="space-y-2">
    ...
    <script>
        ...
        window.transferStaff = {!! json_encode($transferStaff) !!};
    </script>
